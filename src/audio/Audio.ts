// Lightweight audio: try a real file first, otherwise synthesise a placeholder tone with the Web
// Audio API. Sound effects are shared across games (public/audio/sfx/) and a game may override any
// of them (public/modes/<mode>/audio/sfx/); music is per game (public/modes/<mode>/audio/music/).
...
const LS_SFX = "duskhall:vol:sfx";
const LS_MUSIC = "duskhall:vol:music";
const LS_MASTER = "duskhall:vol:master";
const LS_MUTED = "duskhall:audio:muted";
...
const LS_MUSIC_IDX = "duskhall:music:idx";
const LS_MUSIC_POS = "duskhall:music:pos";
const LS_MUSIC_BAG = "duskhall:music:bag";
...
export class AudioEngine {
  ...
  // Reads the SHARED effects manifest (/audio/manifest.json) and the ACTIVE mode's manifest
  // (/modes/<mode>/audio/manifest.json), producing a merged sound→path map (mode effects override
  // shared ones) and the mode's ordered music list. Each manifest is { sfx:[{id,path}], music:
  // [{id,path}] }. Cached per mode asset root so a game switch reloads the new game's music.
  private manifestByRoot = new Map<string, Promise<{ sfx: Map<string, string>; music: string[] }>>();

  private async fetchAudioManifest(url: string): Promise<{ sfx?: unknown; music?: unknown }> {
    try {
  ...
  private loadManifest(): Promise<{ sfx: Map<string, string>; music: string[] }> {
    const modeRoot = `${assetRoot(getActiveMode())}/audio`;
    const cached = this.manifestByRoot.get(modeRoot);
    if (cached) return cached;
    const p = Promise.all([
      this.fetchAudioManifest("/audio/manifest.json"),
      this.fetchAudioManifest(`${modeRoot}/manifest.json`)
    ]).then(([shared, mode]) => {
      const sfx = new Map<string, string>();
      const readSfx = (data: { sfx?: unknown }) => {
      ...
      // Shared first, then the mode's own effects override any of them.
      readSfx(shared);
      readSfx(mode);

      const music: string[] = [];
      if (Array.isArray(mode.music)) {
        for (const e of mode.music as Array<string | { path?: string }>) {
    ...
    this.manifestByRoot.set(modeRoot, p);
    return p;
  }
